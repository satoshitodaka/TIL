...
> [Position](https://getbootstrap.jp/docs/4.1/utilities/position/)
...
- （一例として）_commented_to_own_post.html.slimに、自分の投稿にコメントがあった時の通知を書く
...
## 通知の既読の制御
### app/controller/activities_controller.rb
...
```
- `activity.redirect_path`について、Actibityモデルにメソッドを記述する。
```rb
class Activity < ApplicationRecord
  def redirect_path
